...
> Forked from the original to add fixes needed to work with the Windows11 registry output, and config in Linux Mint 22.1. I do not intend to maintain this futher.

# Dual Boot Bluetooth Pair

This is a tool I use to pair a bluetooth device with my Windows OS (Win 11) and Linux Mint 22.1  OS on my dual boot laptop.
...
```
 ...
 9. From a terminal with `sudo`, navigate to `/var/lib/bluetooth/<ADAPTOR_MAC_ADDRESS>/` and use `ls` to get the mac addresses similar to the bluetooth device you are trying to pair, and re-name the directory to the new mac displayed in the output from step 8. In my case the final digit was incremented by one.
